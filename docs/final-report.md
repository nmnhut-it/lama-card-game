...
**Evaluate whether Claude Code (with Agent Teams) can effectively build a complete HTML5 game on the Cocos2d-JS v3.13 engine** — from zero to deployed — with minimal human intervention. Cocos2d-JS is a legacy framework (last major release 2016) with limited modern documentation, making it a challenging test case for LLM-assisted development.

### Why This Matters

- Cocos2d-JS is still used in production game projects, but has far less training data than React, Unity, or modern web frameworks
- Game development requires visual reasoning, state machine logic, animation timing, and UI polish — areas where LLMs are traditionally weak
- Agent Teams is an experimental feature — we wanted to measure how well multi-agent coordination works on a real project

### Outcome: Positive

Claude Code successfully built and deployed a fully playable card game in **~6 hours** with only **~15 human interventions**. The key findings:

1. **Cocos2d-JS knowledge was sufficient** — the model understood cc.Scene, cc.Layer, cc.DrawNode, cc.LabelTTF, cc.Action, anchor points, z-ordering, and the event system. Minor corrections were needed for anchor point behavior.
2. **Agent Teams worked well for parallel development** — two coders worked on non-overlapping files simultaneously, with a reviewer catching real bugs
3. **Visual polish required human oversight** — text overlap, alignment, and "feel" issues needed human screenshots and feedback
4. **144 tests, all passing** — the model wrote comprehensive unit tests covering game logic, scoring rules, AI decisions, and edge cases
5. **Production-quality code** — clean architecture (game/ai/ui layers), no hard-coded values, all methods under 30 lines, UMD module pattern

### Limitations Observed

- The model occasionally misunderstood Cocos2d anchor point behavior (human corrected once)
- Animation timing and visual "smoothness" required iterative human feedback
- No image/sprite asset creation — all visuals are programmatic (DrawNode)
- Context window limits required 4 automatic continuations during the main session
